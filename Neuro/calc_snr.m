...
function SPIKEZ=calc_snr(RAW,SPIKEZ,flag_simple)
% --- Signal to Noise Ratio (CN,MC) ------------

if nargin < 3
    flag_simple = 0;  % by default, use the original DrCell method (takes longer!)
end


if flag_simple
    % TODO (replace NaN by actual calculation)
    % SNR = zeros(1,size(RAW.M,2));
    % SNR_dB = zeros(1,size(RAW.M,2));
    % COL_RMS = sqrt(mean(RAW.M.^2));
    % COL_SDT = std(RAW.M);
    % SNR_dB = 20*log10(SNR);
