...
function [SBE]=SBEdetection_Sh(BURSTS,rec_dur,SaRa,Th,idleTime,sz)  % faster Algorithm (Sh.Kh) % (BURSTS,rec_dur,SaRa,Th,idleTime)
               ...
               for n=1:size(BURSTS,2)
                    if ~isempty(nonzeros(BURSTS(:,n)))  %only if electrode contains burst
                        for j=1:size(nonzeros(BURSTS(:,n))) 
                            ...
                            binary((start):(ende),2)=1;
                        end 
                         binary(:,1)=sum(binary,2);
                         binary(:,2)=0;
                    end
                end 
