 ...
 * Version: 1.0
 *
 * Function: mexAdaptiveRateIndependentSPIKEDistance
...
#define output1 output[0]
    ...
    if (InN == 3)
    {
        ...
        D = STs->AdaptiveRateIndependentSPIKEdistance(T1[0],THR[0]);
    }
    else
    {
        
        D = STs->AdaptiveRateIndependentSPIKEdistance(T1[0],T2[0],THR[0]);
    }
