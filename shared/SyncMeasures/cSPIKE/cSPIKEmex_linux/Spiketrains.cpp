 ...
 * Other: -
 */
double Spiketrains::AdaptiveSPIKEsynchro(double start,double end,double T1,double T2,double threshold)
{	
    std::vector<std::vector < double> > Synchronized = 
                               AdaptiveSPIKEsynchroProfile(start,end,T1,T2,threshold);
    bool allEmpty = true;
    double sum = 0;
    double Spikes = 0;
 ...
 */
std::vector< std::vector<double> > Spiketrains::AdaptiveSPIKEsynchroProfile(double T1,double T2,double T3,double T4,double threshold)
{
    std::vector< std::vector<double> > Synchronized;
    for (int i = 0; i < STs.size();i++)
    {
            Synchronized.push_back( AdaptiveSynchro(i,T1,T2,T3,T4,threshold) );
    }
    return Synchronized;
 ...
 * Other: -      
 */
std::vector<double> Spiketrains::AdaptiveSynchro(int spiketrainINDEX,double T1,double T2,double T3,double T4,double threshold)
{
        ...
        for (int spike = 1; spike < STs.at(spiketrainINDEX)->Length()-1;spike++)
        {
            ...
            { 
                ...
                for (int otherSTINDEX = 0;otherSTINDEX < STs.size();otherSTINDEX++)
                {
                    ...
                    {
                        sum += AdaptiveCoincidence(spike, spiketrainINDEX,otherSTINDEX,maxwindow,threshold);
                    }
                }
 ...
 * Other: -
 */
double Spiketrains::AdaptiveCoincidence(int NthSpike, int STindex1, int STindex2,double maxwindow,double thr)
{
 ...
 *         -1 if not and 0 if the are at the same time or there is no 
 *         coincidence.
 *
 * Other: -
 */
