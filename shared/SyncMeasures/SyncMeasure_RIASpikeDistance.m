...
function [Sync]=SyncMeasure_RIASpikeDistance(TS,rec_dur, flag_profile) 
    ...
    %% 0) Init
    Sync.S=NaN;
    Sync.PREF.method='RIA-SPIKE-distance';
    ...
    D=STS.AdaptiveRateIndependentSPIKEdistance();
    if flag_profile 
        Sync.PREF.P = NaN; % not available yet: STS.AdaptiveRateIndependentSPIKEdistanceProfile(); 
        Sync.PREF.M = NaN; % not available yet: STS.AdaptiveRateIndependentSPIKEdistanceMatrix();
    end
