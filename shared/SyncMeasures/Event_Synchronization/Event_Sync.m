...
function [es,ed]=Event_Sync(x_spikes,y_spikes,tauMax,precision)

%disp('Event_Sync_Adaptative_40ms')
...
%precision=0.01;%1;            % - logarithm_10 ( sampling interval ) 1: 0.1ms, 0.1: 1ms, 0.01: 10ms

taumode=2;             % how is the allowed time lag defined?            (1-fixed interval (tau),2-adaptive)
timemode=1;            % cumulative, windowed or cumulative normalized?  (1-cumulative,2-windowed,3:cumulative normalized)

tau=3;                  % maximum time delay for which events are still considered to be synchronous (for taumode=1)
window=30;              % window size (for timemode=2)
...
if taumode==1                                       % maximum time lag fixed
    ...
    for efc=1:length(r)-1
        ...
        for efc3=1:length(efc2)
            ...
            tau=min([xmin ymin tauMax]); % 400: 40ms
