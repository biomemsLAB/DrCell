...
function [Sync]=SyncMeasure_RISpikeDistance(TS,rec_dur, flag_profile) 
    ...
    %% 0) Init
    Sync.S=NaN;
    Sync.PREF.method='RI-SPIKE-distance';
    ...
    D=NaN; % does not work???? STS.RateIndependentSPIKEdistance();
    if flag_profile 
        Sync.PREF.P = NaN; % not available yet: STS.RateIndependentSPIKEdistanceProfile(); 
        Sync.PREF.M = NaN; % not available yet: STS.RateIndependentSPIKEdistanceMatrix();
    end
