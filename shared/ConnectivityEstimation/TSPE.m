function [CMres, DMres] = TSPE(sdf, d, neg_wins, co_wins, pos_wins, FLAG_NORM, FLAG_WAITBAR)
% Parameters:
...
if isempty(FLAG_WAITBAR)
    FLAG_WAITBAR=0;
end

if FLAG_WAITBAR; hw = waitbar(0,'Estimating connectivity - Generation of sparse matrices'); end
...
if size(sdf,2)<4000 
    ...
    %mat1=full(mat); %convert sparse into full matrix in order to check
elseif size(sdf,2)>4000 % sparse matrix cannot be created if too many neurons (or recording sites) were used
    M=TS_Cell2M(sdf(1:end-1));
    for i=1:size(M,2)
        mat(:,i)=histcounts(M(:,i), 0:1:NrS);
    end
end


if FLAG_WAITBAR; waitbar(0.1, hw, 'Estimating connectivity - Calculation of std and mean values'); end
...
if FLAG_WAITBAR; waitbar(0.2, hw, 'Estimating connectivity - Fast cross-correlation'); end
...
if FLAG_WAITBAR; waitbar(0.3, hw, 'Estimating connectivity - Generation of edge filter'); end
...
if FLAG_WAITBAR; waitbar(0.4, hw, 'Estimating connectivity - Usage of edge filter'); end

%% Usage of edge filters:
CM(isnan(CM))=0; % MC (if CM contains many NaNs, only NaNs remains after convolution. e.g. for very short spike trains)
for j=1:in
...
if FLAG_WAITBAR; waitbar(0.9, hw, 'Estimating connectivity - Adjustment'); end
...
if FLAG_WAITBAR; waitbar(1, hw, 'Estimating connectivity - Finished'); close(hw); end
