function [Date,Time,SaRa,EL_NAMES,EL_NUMS,M,T,rec_dur,fileinfo,nr_channel]=read_Intan_RHD2000_file(my_path, file)
...
if (data_present)
    ...
    % rename variables to fit DrCell notation
    M=amplifier_data';
    SaRa=frequency_parameters.amplifier_sample_rate;
    rec_dur = t_amplifier(end)+1/SaRa;
    T=t_amplifier;
    Date = 'NA';
    Time = 'NA';
    EL_NAMES = {amplifier_channels.native_channel_name};
    EL_NUMS = [amplifier_channels.native_order];
    fileinfo = {'NA'};
    nr_channel = size(M,2);
...
move_to_base_workspace(my_path);
