...
if nargin < 2
    ...
    %                 numSpikes_temp(n) = length(~isnan(SPIKEZ.TS(:,n)));
    %             end
    %             numSpikes = mode(numSpikes_temp); % select most frequent element
...
if numSpikeColumns > 0 % velocity only can be calculated if there are any spike columns
